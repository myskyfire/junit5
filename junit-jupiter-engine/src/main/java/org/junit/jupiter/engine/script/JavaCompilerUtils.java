...
final class JavaCompilerUtils {
	...
	static class ByteArrayFileObject extends SimpleJavaFileObject {
		...
		ByteArrayFileObject(String canonical, Kind kind) {
			super(URI.create("jupiter:///" + canonical.replace('.', '/') + kind.extension), kind);
		}
	...
	static ClassLoader compile(ClassLoader parent, List<String> options, List<Processor> processors,
		...
		StandardJavaFileManager standardFileManager = compiler.getStandardFileManager(diagnostics, Locale.getDefault(),
